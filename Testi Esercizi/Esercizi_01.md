...
Le funzioni prendono in input un vettore di numeri interi e ritornano rispettivamente la loro media e varianza.


# Esercizio 2

Implementare le funzioni `Mean` e `Variance` affinché possano calcolare media e varianza su vettori di numeri *di qualunque tipo*.
...
Scrivere la funzione `nodup()` che prende in input un vettore di numeri, `v_in`, e ritorna un nuovo vettore, `v_out` contenente gli stessi numeri, ripetuti una sola volta. L'ordine degli elementi in `v_out` non è importante.
